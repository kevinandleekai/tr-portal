        ...
        GLOBAL_CONFIG = {
            pageInitParam: {
                ...
                success: function(data) {
                    ...
                    GLOBAL_CONFIG.listTitle.innerHTML = columnList[0]['columnTitle'];
                    // 填充列表数据
            ...
            // 列表项
            listItems: getByClass('gallery-item')
        };
    ...
    function fillListData(data) {
        ...
        for (; i < len; i++) {
            var currNode = GLOBAL_CONFIG.listItems[i],
                ...
                oP = currNode.getElementsByTagName('p')[0];
            oImg.src = PIC_PATH + (columnList[i]['columnCover'].replace('..', ''));
            oP.innerHTML = columnList[i]['columnTitle'];

            // 添加class
            addClass(currNode, 'gallery-item-div');
        ...
        // 进行组件构建
        var config = {
           nodes: getByClass('gallery-item-div'),
           up: function() {
              var self = this;
